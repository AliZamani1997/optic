#include "LCD.h"
// #include "font1.h"
#include "LCD_Font_16.h"
#include "Fonts/FreeSans9pt7b.h"

const GFXfont * LCD_Font = &FreeSans9pt7b;
...
// ------------ MenuPage ---------------
MenuPage::MenuPage(const String& title, std::vector<MenuItem> items)
    : _title(title), _items(items), _selected(0), _editing(false), _scrollOffset(0) {}
...
void MenuPage::onKey(LCDCustomKeypad k) {
    ...
    if (_editing) {
        ...
        if (item.type == MENU_EDIT_INT) {
            if (k == LCD_KEYPAD_EVENT_LEFT) {
                if (*(item.editInt) > item.editIntMin) (*(item.editInt))--;
            } else if (k == LCD_KEYPAD_EVENT_RIGHT) {
                if (*(item.editInt) < item.editIntMax) (*(item.editInt))++;
    ...
    if (k == LCD_KEYPAD_EVENT_LEFT) {
        ...
        auto& item = _items[_selected];
        if (item.type == MENU_ACTION && item.action) item.action();
        if (item.type == MENU_SUBMENU && item.submenu) item.submenu->onEnter();
        if (item.type == MENU_EDIT_INT || item.type == MENU_EDIT_BOOL) _editing = true;
    }
...
void MenuPage::draw(Adafruit_SSD1306& display) {
    ...
    for (int i = _scrollOffset; i < end; ++i) {
        ...
        if (item.type == MENU_EDIT_INT && item.editInt) {
            ...
            if (_editing && i == _selected) display.print(" [edit]");
        }
...
Popup::Popup(const String& msg, uint32_t timeoutMs)
    : _msg(msg), _timeoutMs(timeoutMs), _startMs(0) {}
...
void ConfirmationPopup::onKey(LCDCustomKeypad k) {
    if (k == LCD_KEYPAD_EVENT_LEFT || k == LCD_KEYPAD_EVENT_RIGHT)
        ...
        if (_callback) _callback(_choice);
...
bool IndustrialLcdUI::begin() {
    ...
    _keypad.begin();
    if(!_display.begin(SSD1306_SWITCHCAPVCC)) return false;
...
void IndustrialLcdUI::loop() {
    ...
    if (!_sleeping) {
        ...
        if (_statusBarProvider) _statusBarProvider(_display);
    }
...
void IndustrialLcdUI::pushPage(UIPage* page) {
    if (!_pageStack.empty()) _pageStack.back()->onExit();
...
void IndustrialLcdUI::popPage() {
    if (!_pageStack.empty()) {
        ...
        _pageStack.pop_back();
        if (!_pageStack.empty()) _pageStack.back()->onEnter();
    }
    wake();
}
...
void IndustrialLcdUI::showPopup(const String& msg, uint32_t timeoutMs) {
    if (_popup) delete _popup;
    _popup = new Popup(msg, timeoutMs);
    _popup->onEnter();
    wake();
}
