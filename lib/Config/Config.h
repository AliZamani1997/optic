...
#define CONFIGLIB_VALUE_MAX_LENGTH 64
#define CONFIGLIB_CRC_LENGTH 2
#define CONFIGLIB_LOG_PATH "/config_changes.log"
#define CONFIGLIB_LOG_FILE(mode, creat) SPIFFS.open(CONFIGLIB_LOG_PATH, mode, creat)
...
public:
    ...
    // String get(config_key_t key, const String& defaultValue);

    // Set value for a key
    bool set(config_key_t key, const String& value);

    // // Initialize a key with a default value if not set
    // void initDefault(config_key_t key, const String& value);
